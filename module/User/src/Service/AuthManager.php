...
class AuthManager
{
     ...
     * to last for one month (otherwise the session expires on one hour).
     */
    public function loginviafacebook($email, $rememberMe)
    {
        // Check if user has already logged in. If so, do not allow to log in
        // twice.
        if ($this->authService->getIdentity()!=null) {
        ...
        $authAdapter->setEmail($email);
     //   $authAdapter->setPassword($password);
        $result = $authAdapter->authenticatefacebookemail();
    ...
    public function loginviatwitter($email, $rememberMe)
    {
        // Check if user has already logged in. If so, do not allow to log in
        // twice.
        if ($this->authService->getIdentity()!=null) {
        ...
        $authAdapter->setEmail($email);
        //   $authAdapter->setPassword($password);
        $result = $authAdapter->authenticatetwitteremail();
    ...
    public function loginviagoogle($email, $rememberMe)
    {
        // Check if user has already logged in. If so, do not allow to log in
        // twice.
        if ($this->authService->getIdentity()!=null) {
        ...
        $authAdapter->setEmail($email);
        //   $authAdapter->setPassword($password);
        $result = $authAdapter->authenticategoogleemail();
    ...
    public function loginvialinkedin($email, $rememberMe)
    {
        // Check if user has already logged in. If so, do not allow to log in
        // twice.
        if ($this->authService->getIdentity()!=null) {
        ...
        $authAdapter->setEmail($email);
        //   $authAdapter->setPassword($password);
        $result = $authAdapter->authenticatelinkedinemail();
